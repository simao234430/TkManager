...
class Prpcrypt(object):
    ...
    @staticmethod
    def get_random_str_6():
        """ 随机生成16位字符串
        @return: 16位字符串
        """
        rule = string.letters + string.digits
        str = random.sample(rule, 6)
        return "".join(str)
...
class WXBizMsgCrypt(object):
    ...
    def encryptMsgJSON(self, sReplyMsg, sNonce, timestamp = None):
        ...
        result["nonce"] = sNonce
        result["appId"] = self.m_sCorpid
        return result

    def decryptMsgJSON(self,  sPostData, sMsgSignature, sTimeStamp, sNonce):
        sha1 = SHA1()
        ...
        if not signature == sMsgSignature:
            ...
            return ierror.WXBizMsgCrypt_ValidateSignature_Error, None
        pc = Prpcrypt(self.key)
        ret,json_content = pc.decrypt(sPostData,self.m_sCorpid)
